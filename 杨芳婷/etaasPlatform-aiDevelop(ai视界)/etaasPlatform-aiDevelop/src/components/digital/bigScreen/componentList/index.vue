...
<script>
import { addCamera, addMessageTip } from "@/utils/digital/createCameraWall";
...
export default {
  data() {
    return {
      oneFlag: false,
      twoFlag: false,
      imgs: require("../../../../assets/digital/assets/img/bigScreen/list.svg"),
      camera: require("../../../../assets/digital/assets/img/camera/camera3.svg"),
    };
  },
  methods: {
    showListOne() {
      ...
      window.$viewer.qtum.draw.deleteAll(),
        window.drawControl.startDraw({
          ...
          success: function (i) {
            ...
            addCamera(dataSource, _that, pointObj);
            // 生成视频流代码（不要了）
            // billboardVideo(
            //   window.$viewer,
            //   Cesium,
      ...
      window.$viewer.qtum.draw.deleteAll(),
        window.drawControl.startDraw({
          ...
          success: function (i) {
            ...
            addMessageTip(dataSource, _that, pointObj);
