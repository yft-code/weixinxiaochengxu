...
               if(element.title === '卫生关注'&&element.switchShow||element.title === '领导视察'&&element.switchShow){
...
               }
    ...
    // 广告牌模式关闭
     closeAllBillboard() {
      console.log('删除广告牌11111111111111111')
       var arr = ['report0Tab','report1Tab','report2Tab','report3Tab']
       arr.map(item=>{
      console.log($viewer.entities.remove({id:item}))
      })
    },
  },
};
