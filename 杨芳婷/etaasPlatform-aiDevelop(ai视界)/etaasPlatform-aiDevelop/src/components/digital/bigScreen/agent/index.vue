<template>
  <div v-if="!$store.state.modelShow" class="agentContainer">
  ...
  watch: {
    locationShow(n, o) {
      if (n === false) {
        ...
        console.log(this.$store.state.angleViewList,'this.$store.state.angleViewList')
        console.log(this.$store.state.checkedAngleViewList,'this.$store.state.checkedAngleViewList')
      }
    },
    menuShow(n, o) {
      if (n === false) {
