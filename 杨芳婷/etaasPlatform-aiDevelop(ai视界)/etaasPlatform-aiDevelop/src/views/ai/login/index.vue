          ...
          >
            ...
            <el-form-item prop="code">
              <el-input
                ...
                class="codeInput"
              ></el-input>
              <img :src="imgSrc" class="codeImg" @click="getCode" />
              <div class="clear"></div>
            </el-form-item>
            <el-button
              type="primary"
...
import { getNoticemNum } from "@/api/ai/earlyWarning";
export default {
  components: { voice },
  name: "Login",
  data() {
  ...
  methods: {
    ...
    // 跳转到首页第一个显示菜单
    handleToPage() {
      // this.$router.replace({ path: "/consoleDesk" });
          ...
          //     // 登录以form-data形式传参
              ...
              api
                ...
                .then((res) => {
                  ...
                  initWebSocket(userInfo.aiHorizon, token);
    ...
    fetchNoticeData() {
      ...
      if (localStorage.getItem("token")) {
        getNoticemNum()
          .then((res) => {
            that.$bus.$emit("number", res.result);
          })
          .catch((res) => {});
      }
    },
